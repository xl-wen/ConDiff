...
class GaussianDiffusion(nn.Module):
    def __init__(self, mean_type, noise_schedule, noise_scale, noise_min, noise_max, \
        ...
        self.noise_schedule = noise_schedule  # 'linear_var'
        ...
        # shape [steps, history_num_per_term]
    ...
    def p_sample(self, model, x_start, steps, sampling_noise=False):
        ...
        if self.noise_scale == 0.:
            for i in indices:
                ...
                x_t = model(x_t, x_start, ts)
            return x_t

        for i in indices:
            t = th.tensor([i] * x_t.shape[0]).to(x_start.device)
            out = self.p_mean_variance(model, x_t, x_start, t)
            if sampling_noise:
    ...
    def training_losses(self, model, x_start, reweight=False):
        ...
        terms = {}
        model_output = model(x_t, x_start, ts)
        target = {
    ...
    def p_mean_variance(self, model, x, x_start, t):
        """
        ...
        B, C = x.shape[:2]
        assert t.shape == (B,)
        model_output = model(x, x_start, t)
